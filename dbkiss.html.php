<?php
/*
	DBKiss 1.09 (2011-01-08)
	Author: Cezary Tomczak [cagret@gmail.com]
	Web site: http://www.gosu.pl/dbkiss/
	License: BSD revised (free for any use)
	
	Modified by ejr, VoiceNation LLC for FreePBX module dbeditor
*/
// ejr 2/3/11 - FreePBX turns on cache-control just before including this file.  We need to disable it.
...
// ejr 2/4/11 - tried setting the cookie_prefix like the above example and it breaks the flash cookie message on delete row (which prevents parent window refresh)
// by using the default value, all works well
// this default copied from below but needed before we auto-set the db values into the cookie
...
$cookie = array(
	'db_driver' => $amp_conf['AMPDBENGINE'],
	'db_server' => $amp_conf['AMPDBHOST'],
...
if (!ini_get('date.timezone')) {
	// ejr - 2/3/11 changed timezone to New York
...
function conn_modify($key)
{
	if (array_key_exists($key, $_GET)) {
		...
		if (isset($_GET['from']) && $_GET['from']) {
			...
			header('Location: '.$_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php');
		}
		exit;
	}
}
...
if (!$db_pass || (!$db_driver || !$db_server || !$db_name || !$db_user))
{
	...
	?>
		...
		<head>
			...
			<title>Connect</title>
			<link rel="shortcut icon" href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php'; ?>&dbkiss_favicon=1">
		</head>
		<body>
		...
		<form action="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>" method="post">
	...
	foreach($text as $key => $value)
	{
		...
		if(!$result) {
			...
			if (!$ignore_errors) {
				...
				echo '<br><div><a href="'.$_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php'.'&import=1">&lt;&lt; go back</a></div>';
				exit;
			}
		}
	}
...
function conn_info()
{
	...
	<p>
		...
		&nbsp;-&nbsp;
		Database: <select name="db_name" onchange="location='<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&db_name='+this.value"><?php echo options($dbs, $db_name);?></select>
		&nbsp;-&nbsp;
		<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&execute_sql=1">Execute SQL</a>
		(
		<a href="javascript:void(0)" onclick="popup('<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&execute_sql=1&popup=1')">popup</a>
		)
		&nbsp;-&nbsp;
		Db charset: <select name="db_charset" onchange="location='<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&db_charset='+this.value+'&from=<?php echo urlencode($_SERVER['REQUEST_URI']);?>'">
		<option value=""></option><?php echo options($charset1, $db_charset);?></select>
		&nbsp;-&nbsp;
		Page charset: <select name="page_charset" onchange="location='<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&page_charset='+this.value+'&from=<?php echo urlencode($_SERVER['REQUEST_URI']);?>'">
		<option value=""></option><?php echo options($charset2, $page_charset);?></select>
		&nbsp;-&nbsp;
		<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&disconnect=1">Disconnect</a>
	</p>
	<?php
}
function size($bytes)
...
function url($script, $params = array())
{
	...
	// ejr 2/3/11 was only the 'else' code
	if (strpos($script, 'dbkiss.html.php') === false) {	
		$query = str_replace("?","&",$query); // don't repeat the '?' we're setting below
		return $script.'?handler=file&module=dbeditor&file=dbkiss.html.php'.$query;
	}
	else {
		return $script.$query;		
	}
...
function flash()
{
	...
	<div id="flash" ondblclick="document.getElementById('flash').style.display='none';">
		...
		<?php // ejr - 2/3/11 - if we were redirected with an ok then we updated a record - refresh the parent list of rows too ?>
		<?php if($flash_ok) echo('<script type="text/javascript">window.opener.location.reload();</script>'); ?>
	</div>
...
<?php if (get('import')): ?>
	...
	<?php layout(); ?>
	<h1><a style="<?php echo $db_name_style;?>" href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>"><?php echo $db_name_h1?$db_name_h1:$db_name;?></a> &gt; Import</h1>
	<?php conn_info(); ?>
...
?>
	<?php if (is_numeric($found)): ?>
		<p>
			...
			<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php'; ?>&export=csv&separator=<?php echo urlencode('|');?>&query=<?php echo base64_encode($base_query); ?>">pipe</a>
			-
			<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php'; ?>&export=csv&separator=<?php echo urlencode("\t");?>&query=<?php echo base64_encode($base_query); ?>">tab</a>
			-
			<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php'; ?>&export=csv&separator=<?php echo urlencode(',');?>&query=<?php echo base64_encode($base_query); ?>">comma</a>
			-
			<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php'; ?>&export=csv&separator=<?php echo urlencode(';');?>&query=<?php echo base64_encode($base_query); ?>">semicolon</a>
		</p>
	<?php else: ?>
...
<?php layout_start(($db_name_h1?$db_name_h1:$db_name).' &gt; Execute SQL'); ?>
	...
	<?php else: ?>
		<h1><a style="<?php echo $db_name_style;?>" href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>"><?php echo $db_name_h1?$db_name_h1:$db_name;?></a> &gt; Execute SQL</h1>
	<?php endif; ?>
	...
	<form action="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&execute_sql=1&popup=<?php echo $get['popup'];?>" method="post" onsubmit="return sql_submit(this);" style="margin-top: 1em;">
...
<body>
	...
	<h1><a style="<?php echo $db_name_style;?>" href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>"><?php echo $db_name_h1?$db_name_h1:$db_name;?></a> &gt; Table: <?php echo $table;?></h1>

	<?php conn_info(); ?>

	<p>
		<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>">All tables</a>
		&nbsp;&gt;&nbsp;
		<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&viewtable=<?php echo $table;?>"><b><?php echo $table;?></b></a> (<?php echo $count;?>)
		&nbsp;&nbsp;/&nbsp;&nbsp;
		...
		&nbsp;&nbsp;/&nbsp;&nbsp;
		Functions:
		<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&viewtable=<?php echo $table;?>&indenthead=1">indenthead()</a>
	</p>

	<form action="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>" method="get" style="margin-bottom: 1em;">
	<?php // ejr 2/4/11 - these allow processing through FreePBX file handler ?>
	<input type="hidden" name="handler" value="file">
	<input type="hidden" name="module" value="dbeditor">
	<input type="hidden" name="file" value="dbkiss.html.php">
...
<?php layout_start(sprintf('%s &gt; Search', $db_name)); ?>
	<h1><a style="<?php echo $db_name_style;?>" href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>"><?php echo $db_name_h1?$db_name_h1:$db_name;?></a> &gt; Search</h1>
	<?php conn_info(); ?>

	<form action="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>" method="get">
	<?php // ejr 2/4/11 - these allow processing through FreePBX file handler ?>
	<input type="hidden" name="handler" value="file">
	<input type="hidden" name="module" value="dbeditor">
	<input type="hidden" name="file" value="dbkiss.html.php">
	...
	<?php if ($get['search'] && count($get['types'])): ?>
		...
		<?php foreach ($tables as $table): ?>
			...
			<?php if ($where && $found): ?>
				...
				<p>
					Table: <a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&viewtable=<?php echo $table;?>&search=<?php echo urlencode($get['search']);?>"><b><?php echo $table;?></b></a><br>
					Found: <b><?php echo $found;?></b>
					<?php if ($found > $limit): ?>
						&nbsp;<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&viewtable=<?php echo $table;?>&search=<?php echo urlencode($get['search']);?>">show all &gt;&gt;</a>
					<?php endif; ?>
				</p>
...
<p>
	...
	&nbsp;-&nbsp;
	<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&searchdb=1&table_filter=<?php echo html_once($get['table_filter']);?>">Search</a>
	&nbsp;-&nbsp;
	<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&import=1">Import</a>
	&nbsp;-&nbsp;
	Export all:
	&nbsp;
	<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&dump_all=1&table_filter=<?php echo urlencode(html_once($get['table_filter']));?>">structure</a>
	&nbsp;/&nbsp;
	<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&dump_all=2&table_filter=<?php echo urlencode(html_once($get['table_filter']));?>">structure & data</a>
...
<form action="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>" method="get" style="margin-bottom: 0.5em;">
<?php // ejr 2/4/11 - these allow processing through FreePBX file handler ?>
<input type="hidden" name="handler" value="file">
<input type="hidden" name="module" value="dbeditor">
<input type="hidden" name="file" value="dbkiss.html.php">
...
<?php foreach ($tables as $table): ?>
<tr>
	<td><a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&viewtable=<?php echo $table;?>"><?php echo $table;?></a></td>
	<?php
		if ('mysql' == $db_driver) {
			// $table_enq = quote_table($table);
			// $count = db_one("SELECT COUNT(*) FROM $table_enq");
	...
	<td>
		<a href="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>&dump_table=<?php echo $table;?>">Export</a>
		&nbsp;-&nbsp;
		<?php $table_enq = quote_table($table); ?>
		<form action="<?php echo $_SERVER['PHP_SELF'].'?handler=file&module=dbeditor&file=dbkiss.html.php';?>" name="drop_<?php echo $table;?>" method="post" style="display: inline;">
		<input type="hidden" name="drop_table" value="<?php echo $table;?>">
		</form>
